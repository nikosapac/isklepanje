...
public class NajboljZajeten {
	private WebDriver driver;
	public NajboljZajeten (WebDriver driver) {
		this.driver = driver;
	}
	public void maleZivaliPrviTest() throws InterruptedException {
        ...
        calendar.add(Calendar.DAY_OF_MONTH, -27); // Subtract half a month (15 days)
        Date toYoungDate = calendar.getTime();
        SimpleDateFormat dateFormat = new SimpleDateFormat("MM-dd-yyyy");
        String toYoungDateStr = dateFormat.format(toYoungDate);
        ...
        calendar.add(Calendar.MONTH, -1); // Subtract one month
        calendar.add(Calendar.DAY_OF_MONTH, -27); // Subtract half a month (15 days)
        Date toOldDate = calendar.getTime();
        SimpleDateFormat dateFormatOld = new SimpleDateFormat("MM-dd-yyyy");
        String toOldDateStr = dateFormatOld.format(toOldDate);
		...
		int triglavKomplet = 0; // 1-yes ; 0-no		To bo preverilo ali so izpisani procenti na desni strani strani
		...
		isDateElementNotClickable(driver);	//checking date element is not clickable
			...
			//Typing in animal info, posamicno in preverjanje za error message
		String yesNO = null;
		int verydangerous = 0;
		int zzz = 0;
		if (pasemska == 3) {
		...
		f.ScrollInKlik2(By.xpath("(//*[@class='radio--primary__button'])["+pasemska+"]"));
			...
			//choosing breed & dangerous breed
				...
				if (z == 2)	{			//authorized veterinarian
		...
		if (z == zzz-1) {
			driver.findElement(By.id("pet.microchipNum")).sendKeys("5656");
			f.ScrollInKlik2(By.className("submitButton"));
			driver.findElement(By.id("pet.microchipNum.errors"));
			driver.findElement(By.id("pet.microchipNum")).clear();
			//to young
			driver.findElement(By.id("pet.birthDate")).clear();
			f.ScrollToElement(By.id("pet.birthDate"));
			driver.findElement(By.id("pet.birthDate")).sendKeys(toYoungDateStr);
			f.ScrollInKlik2(By.className("submitButton"));
			driver.findElement(By.id("pet.birthDate")).clear();
			driver.findElement(By.id("pet.birthDate")).sendKeys(toOldDateStr);
			f.ScrollInKlik2(By.className("submitButton"));
			driver.findElement(By.id("pet.birthDate")).clear();
		}
